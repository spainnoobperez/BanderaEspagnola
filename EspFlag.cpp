...
public:
    ...
    Vec3& getNormal() {
        return accumulated_normal;   // Normal on each particle
    }
...
private:
    ...
    Mass* getParticle(int x, int y) {
        return &particles[y*num_particles_width + x];   //Get particle at row y and column x
    }
    void makeSpring(Mass *p1, Mass *p2) {
        springs.push_back(Spring(p1,p2));   //Add elements to the spring list
    }
...
public:
    ...
    void drawShaded() {
        ...
        for(int y=0; y<num_particles_height-1; y++) {
            for(int x = 0; x<num_particles_width-1; x++) {
                Vec3 color(0,0,0);
                // Spain flag RGB Properties: EX: #aa151b RGB: (170, 21, 27), HEX: #f1bf00 RGB: (241, 191, 0), HEX: #aa151b RGB: (170, 21, 27)
                if(y < num_particles_height / 4)
                    color = Vec3(0.6666666f, 0.082352f, 0.105882f);
                else if(y < (num_particles_height / 4) + (num_particles_height / 2))
                    color = Vec3(0.945098f, 0.749019f, 0);
                else if(y < num_particles_height)
                    color = Vec3(0.6666666f, 0.082352f, 0.105882f);
...
float ball_radius = 3; // the radius of our one ball
...
void init() {
    ...
    quad = gluNewQuadric();
    Image* image1 = loadBMP(IDB_BITMAP1);
    _textureId1 = loadTexture(image1);
    delete image1;
    Image* image2 = loadBMP(IDB_BITMAP2);
...
void display(void) {
    ...
    gluSphere(quad,ball_radius-0.1,50,50);
    glPopMatrix();
    glutSwapBuffers();
    glutPostRedisplay();
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd) {
    ...
    glutCreateWindow("El Imperio espanol nunca decaerá!");
    init();
